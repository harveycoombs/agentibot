...
def check_guild_interaction_limit_hit(guild_id):
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) AS n FROM registered_guilds WHERE interactions_this_month >= 200 AND interaction_start_date >= NOW() - INTERVAL '1 month' AND guild_id = %s", (guild_id,))
        result = cursor.fetchone()
...
def insert_error_log(guild_id, author_id, prompt, error_message):
    ...
    try:
        ...
        cursor.execute("INSERT INTO errors (error_id, incident_date, guild_id, author_id, prompt, error) VALUES(gen_random_uuid(), NOW(), %s, %s, %s, %s)", (guild_id, author_id, prompt, error_message))
        connection.commit()
